...
!
  integer, parameter:: ma = 10    ! Rows of Matrix A
  integer, parameter:: na = 20    ! Columns of Matrix A
  integer, parameter:: mb = 20    ! Rows of Matrix B
  integer, parameter:: nb = 25    ! Columns of Matrix B
!
...
!
  ...
  if ( my_rank == 0 ) then
    ...
    if ( num_procs < 2 ) then
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'Matrix Multiply-Fatal error!'
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a, i8, a)' ) '  Process ', my_rank, ' is active.'
!
...
!
  if ( my_rank == 0 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'Matrix Multiplication-Master process:'
...
!  st = MPI_Wtime()
  ...
  if ( my_rank == 0 ) then
    ...
    do i = 1, mb 
      write ( 10, '(20g10.4)' ) c(i, 1:mb)
      open(10, file = 'mxm.txt', status = 'old')
    end do

!  endx = MPI_Wtime()
!  write (*,'(a)') 'Elapsed time', st-endx, 'seconds'
  end if
